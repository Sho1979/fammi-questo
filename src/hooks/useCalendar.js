...
export async function addEvent(data) {
  ...
  if (!familyId) {
    throw new Error('addEvent: familyId mancante — utente non loggato?')
  }
  const record = await createRecord('events', { ...data, family_id: familyId })
  // Schedula reminder nativo (noop su web) — protetto da try/catch
  try {
    scheduleEventReminder(record)
  } catch (err) {
    console.warn('[Calendar] scheduleEventReminder fallito (evento comunque salvato):', err)
  }
  console.log('[Calendar] Evento salvato:', record.id, record.title, record.date, record.time_start)
  return record
}
...
export async function updateEvent(id, changes) {
  ...
  // Re-schedula reminder con i nuovi dati — protetto
  try {
    scheduleEventReminder(record)
  } catch (err) {
    console.warn('[Calendar] scheduleEventReminder fallito su update:', err)
  }
  return record
}
