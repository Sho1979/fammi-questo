...
function RootRedirect() {
  ...
  const fullReset = useAuthStore((s) => s.fullReset)
  const [checking, setChecking] = useState(true)

  // Double-check: if authStore says setup is complete but Dexie has no members,
  // force re-setup (handles fresh install with stale localStorage)
  useEffect(() => {
    async function verify() {
      if (isSetupComplete && familyId) {
        try {
          const count = await db.members
            ...
            .count()
          if (count === 0) {
            // No members in DB → stale state, force re-setup
            fullReset()
          }
        } catch {
          // DB error → let it proceed, setup page will handle
        }
      }
      setChecking(false)
    }
    verify()
  }, [isSetupComplete, familyId, fullReset])
