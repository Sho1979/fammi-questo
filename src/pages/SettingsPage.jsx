...
export default function SettingsPage() {
  ...
  const handleResetFinal = async () => {
    ...
    // Clear ALL tables — synced + local-only (nessuna tabella dimenticata)
    await Promise.all([
      db.family.clear(),
      db.members.clear(),
      db.expenses.clear(),
      ...
      db.patterns.clear(),
      db.nlpDocuments.clear(),
      db.nlpLogs.clear(),
      db.conversationDrafts.clear(),
    ...
    <div className="flex flex-col gap-3.5 px-4 py-4 pb-24" style={{ background: 'var(--bg-main)' }}>
      ...
      <div className="rounded-2xl bg-white p-4 shadow-sm border border-gray-100">
        ...
        <p className="text-xs text-gray-400 mb-1">
          {members.filter(m => m.role === 'parent').length} genitori,{' '}
          {members.filter(m => m.role === 'elder').length} nonni,{' '}
      ...
      <div className="rounded-2xl bg-white p-4 shadow-sm border border-gray-100">
        ...
        {isSyncEnabled() ? (
          <>
            ...
            {/* Sync button — React 19 useOptimistic for instant feedback */}
            <button
              type="button"
              onClick={() => startSync('full')}
              disabled={syncState !== SYNC_STATES.IDLE && syncState !== SYNC_STATES.SUCCESS && syncState !== SYNC_STATES.ERROR}
              className={`w-full flex items-center justify-center gap-2 rounded-xl px-4 py-2.5
                text-xs font-semibold text-white shadow-md transition-all mb-2
            ...
            >
              {syncState === SYNC_STATES.SUCCESS ? (
                <><CheckCircle size={14} /> Sincronizzato!</>
              ) : syncState === SYNC_STATES.ERROR ? (
                <><AlertCircle size={14} /> Errore — riprova</>
              ) : syncState === SYNC_STATES.PUSHING || syncState === SYNC_STATES.PULLING ? (
                <>
            ...
            {lastSyncAt && (
              <p className="text-[11px] text-gray-400 text-center mb-2">
                Ultima sync: {new Date(lastSyncAt).toLocaleString('it-IT', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit' })}
              </p>
            )}
            {syncError && syncState === SYNC_STATES.ERROR && (
              <p className="text-[11px] text-red-400 text-center mb-2">{syncError}</p>
            )}
