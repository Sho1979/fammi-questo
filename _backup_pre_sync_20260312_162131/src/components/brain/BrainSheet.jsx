        ...
        {phase === 'done' && (() => {
          ...
          return (
            <div className="flex flex-col items-center gap-3 py-6 px-6">
              ...
              {/* Title */}
              <p className="text-base font-semibold" style={{ color: 'var(--text-primary)' }}>
                {allFailed ? 'Errore' : allOk ? 'Fatto!' : 'Completato con avvisi'}
              </p>
                  ...
                  {executionLog.map((log, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-2 text-xs">
                        {isWarning
                          ? <Link2Off size={12} className="text-amber-500 flex-shrink-0" />
                          : log.ok
                            ? <CheckCircle2 size={12} className="text-emerald-500 flex-shrink-0" />
                            : <XCircle size={12} className="text-red-500 flex-shrink-0" />
                        }
                        <span style={{ color: isWarning ? 'var(--text-warning, #b45309)' : 'var(--text-secondary)' }}>
                          {log.msg}
                        </span>
                      </div>
                    )
