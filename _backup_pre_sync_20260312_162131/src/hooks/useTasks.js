...
import { scheduleTaskReminder, cancelTaskReminder } from '../lib/notificationScheduler.js'
...
export async function addTask(data) {
  ...
  // Schedula reminder nativo per la scadenza (noop su web)
...
export async function updateTask(id, changes) {
  ...
  // Re-schedula se la data è cambiata
  if (changes.due_date !== undefined) scheduleTaskReminder(record)
  return record
}
...
export function useWeeklyLeaderboard(familyId) {
  ...
  return useLiveQuery(
    ...
    [familyId],
    { leaderboard: [], weekLabel: '', weeklyPrize: WEEKLY_PRIZE, isParimerito: false, weeklyPrizeEach: WEEKLY_PRIZE, topScore: 0 }
  )
}
