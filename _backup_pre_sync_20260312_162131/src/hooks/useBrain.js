...
// Rate limit: max 20 AI calls/day (local parse is unlimited)
let dailyAICalls = 0
const MAX_DAILY_AI_CALLS = 20

// Init flags (una volta per sessione)
let decayApplied = false
let nlpInitStarted = false
...
export default function useBrain() {
  ...
  useEffect(() => {
    ...
    // Init NLP.js (una volta per sessione)
    if (!nlpInitStarted) {
      nlpInitStarted = true
      console.log('[Brain] Avvio inizializzazione NLP.js...')
      initNlp(familyId).then((ready) => {
        setNlpReady(ready)
      ...
      // Se già iniziato, controlla stato
      setNlpReady(isNlpReady())
    }
  }, [familyId])
  ...
  const doParse = async (text) => {
    ...
    try {
      ...
      const context = {
        members: membersList.map(m => ({ name: m.name, role: m.role, id: m.id })),
        currentMember,
        familyId,
      }
  ...
  const confirmActions = useCallback(async (confirmedActions) => {
    ...
    for (const action of independent) {
      try {
        ...
        console.error('[Brain] Action failed:', action, err)
    ...
    for (const action of dependent) {
      ...
      try {
        ...
        console.error('[Brain] Action (linked) failed:', action, err)
    ...
    switch (action.type) {
      ...
      case 'calendar': {
        ...
        const record = await addEvent({
          ...
          category: action.category || 'altro',
          person_id: action.personIds?.[0] || currentMember?.id,
          note: eventNote,
          logistics,
  ...
  return {
    ...
    // Info
    aiCallsRemaining: MAX_DAILY_AI_CALLS - dailyAICalls,
